...
function [Genome] = RandomGenome(Min,Max)
    NGenes=length(Min);
    Genome=zeros(1,NGenes);
    
    for gen=1:NGenes
%         RandGen = (Max(gen)+Min(gen))/2+abs(randn(1))/3*Max(gen)+abs(randn(1))/3*Min(gen);
        RandGen = Min(gen)+rand(1)*(Max(gen)-Min(gen));
        Genome(gen)=max(min(RandGen,Max(gen)),Min(gen));
    end
    
    Genome=CheckGenome(Genome);
end

function [Genome] = CheckGenome(Genome)
    % Make sure that a torque pulse doesn't end after the next spike
    for ge=1:NumTorques
...
function [NewGenome] = CrossOver(Genome1,Genome2)
    NGenes=length(Genome1);
    
    % Choose 2 crossover points at random
    Points=sort(ceil(rand(1,2)*(NGenes-1)));
    if Points(2)==Points(1)
        Points(2)=Points(2)+1;
    end
    
    Bits1=[zeros(1,Points(1)) ones(1,NGenes-Points(1))];
    Bits2=[zeros(1,Points(2)) ones(1,NGenes-Points(2))];
    Bits=xor(Bits1,Bits2);
            
    NGenes=length(Genome1);
    NewGenome=Genome1;
    for gen=1:NGenes
        if Bits(gen)==1
            NewGenome(gen)=Genome2(gen);
        end
    end
end
...
function [NewGenome] = Mutate(Genome,Min,Max,Prob)
    NGenes=length(Genome);
    
    NewGenome=Genome;
    for gen=1:NGenes
        if rand(1)<=Prob
            NewGenome(gen)=NewGenome(gen)+randn()/3*(Max(gen)-Min(gen))*min(max(Prob,0.5),0.05);
        end
    end
    
    NewGenome=CheckGenome(NewGenome);            
end
